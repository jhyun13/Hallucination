...
from prompts.cove_prompt import REVISION_PROMPT
import pandas as pd
import torch
import random
...
class Reviser:
    ...
    def revise_text(self, data: pd.DataFrame):
        ...
        input_text_list = data['input_text']
        plan_list = data['plan']
        ...
        for input_text, plan, reasoning, agreement in tqdm(zip(input_text_list, plan_list, reasoning_list, agreement_list)):
            start_time = time.time()
            
            if agreement == "Hallucination":
                revision_prompt = REVISION_PROMPT % (input_text, plan, reasoning)
